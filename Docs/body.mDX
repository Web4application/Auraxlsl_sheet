...
<configuration>
    <packageSources>
        <clear />
        <add key="github" value="https://nuget.pkg.github.com/auraxlsl_sheet/index.json" />
    </packageSources>
    <packageSourceCredentials>
        <github>
            <add key="Username" value="web4application" />
...
```shell
dotnet nuget add source --username web4application --password YOUR_GITHUB_PAT --store-password-in-clear-text --name github "https://nuget.pkg.github.com/web4application/index.json"
```
...
1. Create a new project. Replace `PROJECT_NAME` with the name you'd like to give the project.
   ...
   ```shell
   dotnet new console --name auraxlsl_sheet
   ```
...
3. Publish the package using your personal access token as the API key. Replace `PROJECT_NAME` with the name of the project, `1.0.0` with the version number of the package, and `YOUR_GITHUB_PAT` with your personal access token.
   ...
   ```shell
   dotnet nuget push "bin/Release/auraxlsl_shee.1.2.0.nupkg" --api-key YOUR_GITHUB_PAT --source "github"
   ```
   ...
   * `REPOSITORY` with the name of the repository to which you want to connect your package.
   ...
   <Project Sdk="Microsoft.NET.Sdk">
     ...
     <PropertyGroup>
       ...
       <PackageId>auraxlsl_sheet</PackageId>
       <Version>1.2.0</Version>
       <Authors>seriki yakub</Authors>
       <Company>qubuhub</Company>
...
2. To use a package, add `ItemGroup` and configure the `PackageReference` field in the *.csproj* project file. Replace the `PACKAGE_NAME` value in `Include="PACKAGE_NAME"` with your package dependency, and replace the `X.X.X` value in `Version="X.X.X"` with the version of the package you want to use:
   ...
   <Project Sdk="Microsoft.NET.Sdk">
     ...
     <PropertyGroup>
       <OutputType>Exe</OutputType>
       <TargetFramework>netcoreapp3.0</TargetFramework>
       <PackageId>xlslpaper</PackageId>
       <Version>1.2.0</Version>
      <Authors>seriki yakub</Authors>
       <Company>GitHub</Company>
      <PackageDescription>This package adds an Octocat!</PackageDescription>
       <RepositoryUrl>https://github.com/web4application/auraxlsl_sheet</RepositoryUrl>
     </PropertyGroup>

     <ItemGroup>
       <PackageReference Include="auraxlsl_sheet" Version="1.2.0" />
     </ItemGroup>
